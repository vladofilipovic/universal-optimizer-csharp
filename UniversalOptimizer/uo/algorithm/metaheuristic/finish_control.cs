namespace uo.Algorithm.Metaheuristic
{
    ...
    public class FinishControl
    {
        ...
        {
            return this.CheckEvaluations && evaluation >= this.EvaluationsMax || this.CheckIterations && iteration >= this.IterationsMax ||
              this.CheckSeconds && elapsedSeconds >= this.SecondsMax;
        }
            ...
            string groupEnd = "}")
        {
            ...
            s += "evaluationsMax=" + this.EvaluationsMax.ToString() + delimiter;
            foreach (var i in Enumerable.Range(0, indentation - 0))
            {
                s += indentationSymbol;
            }
            s += "iterationsMax=" + this.IterationsMax.ToString() + delimiter;
            foreach (var i in Enumerable.Range(0, indentation - 0))
            {
                s += indentationSymbol;
            }
            s += "secondsMax=" + this.SecondsMax.ToString() + delimiter;
            foreach (var i in Enumerable.Range(0, indentation - 0))
            {
                s += indentationSymbol;
            }
